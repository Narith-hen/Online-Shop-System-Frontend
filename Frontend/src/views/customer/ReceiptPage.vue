<template>
  <div class="max-w-3xl mx-auto px-4 py-8">
    ...
    <div v-else-if="error" class="text-center py-16">
      ...
      <router-link to="/orders" class="inline-block bg-blue-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-blue-700 transition">
        Back to Orders
      </router-link>
    </div>

    <div v-else-if="order" class="bg-white rounded-lg shadow-md p-6 sm:p-8" id="receipt">
      <!-- Receipt Header -->
      <div class="border-b border-gray-200 pb-6 mb-6 flex flex-col sm:flex-row justify-between items-start gap-4">
        <div>
          ...
          <p class="text-gray-500 text-sm mt-1">Order #{{ order.id }}</p>
        </div>
      ...
      <div class="border-b border-gray-200 pb-6 mb-6">
        ...
        <p class="text-sm text-gray-600">Name: {{ order.customer_name || order.user?.name || 'N/A' }}</p>
        <p class="text-sm text-gray-600">Email: {{ order.customer_email || order.user?.email || 'N/A' }}</p>
        <p class="text-sm text-gray-600" v-if="order.payment_method">
      ...
      <div class="border-b border-gray-200 pb-6 mb-6">
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            <tr v-for="item in order.items" :key="'status-'+item.id">
              <td colspan="4" class="pb-2 text-xs text-gray-400 pl-4" v-if="item.status !== 'pending'">
    ...
    <!-- Actions -->
    <div v-if="order" class="flex flex-col sm:flex-row gap-3 mt-6 justify-center">
      <button @click="printReceipt" class="bg-gray-900 text-white font-bold py-3 px-8 rounded-lg hover:bg-gray-800 transition">
        <i class="fas fa-print mr-2"></i>Print Receipt
      </button>
      <router-link to="/orders" class="bg-blue-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-blue-700 transition text-center">
        Back to Orders
      </router-link>
    </div>
  </div>
